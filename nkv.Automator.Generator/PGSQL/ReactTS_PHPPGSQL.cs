...
namespace nkv.Automator.Generator.PGSQL
{
    public class ReactTS_PHPPGSQL
    {
        ...
        public ReactTS_PHPPGSQL(string projectName, string destinationFolder, string destinationFolderSeparator)
        {
            DestinationFolder = destinationFolder;
            ProjectName = projectName;
            ExList = new List<Exception>();
            TemplateFolder = "ReactPHPTsTemplate";
            ...
            foreach (var r in ReactRepoDic)
            {
                try
                {
                    ...
                    foreach (var f in r.Value)
                    {
                        ...
                        {
                            //paramList = tableData.PrimaryKeyCommaString + ",data";
                            paramList = "data";
                            paramData = ",data";
                        }
                        else if (f.functionType == "getbyid")
                        {
                            //paramList = tableData.PrimaryKeyCommaString;
                        ...
                        else if (f.functionType == "delete")
                        {
                            ...
                            {
                                if (!string.IsNullOrEmpty(PrimaryKeyCommaString))
                                {
                                    PrimaryKeyCommaString = PrimaryKeyCommaString + "," + p.FieldName;
                                    paramData = paramData + "," + p.FieldName + ":" + p.FieldName;
                                }
                                else
                                {
                                    PrimaryKeyCommaString = p.FieldName;
                                    paramData = p.FieldName + ":" + p.FieldName;
                                }
                            }
        ...
        private void CreateAuthServiceFile()
        {
            ...
            string tokenURL = "/token/generate.php";
            using (var txtFile = File.AppendText(path))
            {
                ...
                try
                {
                    ...
                    foreach (var c in r.Value.SelectQueryData.ColumnList)
                    {
                        if (!string.IsNullOrEmpty(ColumnListWithValue))
                            ColumnListWithValue = ColumnListWithValue + "," + c.Field + ":''";
                        else
                            ColumnListWithValue = c.Field + ":''";
                    }
        ...
        public List<Exception> CreateReactAPP(ReactJSInput<FinalDataPHP> reactInput)
        {
            CreateDirectory();
