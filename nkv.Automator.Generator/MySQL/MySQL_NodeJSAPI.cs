...
namespace nkv.Automator.Generator.MySQL
{
    public class MySQL_NodeJSAPI
    {
        ...
        public MySQL_NodeJSAPI(NKVConfiguration config, bool isMultiTenant, string destinationFolderSeparator)
        {
            IsMultiTenant = isMultiTenant;
            ConfigApp = config;
            ExceptionList = new List<Exception>();
            TemplateFolder = "NodeJSAPITemplate";
        ...
        public ReactJSInput<FinalQueryData> Automator(string projectName, List<string> selectedTable, MySQLDBHelper mySQLDB)
        {
            ...
            foreach (string tableName in selectedTable)
            {
                try
                {
                    ...
                    var finalData = mysqlDB.BuildQueryNodeJS(tableName);
        ...
        public void CreateEnvFiles()
        {
            ...
            using (var txtFile = File.AppendText(path))
            {
                contents = contents.Replace("{host}", mysqlDB.Host);
                contents = contents.Replace("{dbUser}", mysqlDB.Username);
        ...
        public void CreateTokenFiles()
        {
            ...
            if (isAuth)
            {
                ...
                userCheck = "const query = `SELECT  t.* FROM " + authTable + " t  WHERE t." + username + "=? AND t." + password + "=? LIMIT 0,1`;" + Environment.NewLine;
        ...
        public PostmanModel CreatePostmanJson(string tableName, string modelName, string type, FinalQueryData data, string fkColumnName = "")
        {
            ...
            //if (type == "update" && type == "add" && type == "patch")
            //{
            //    if (data.SelectQueryData.PrimaryKeys != null && data.SelectQueryData.PrimaryKeys.Count > 1)
            //    {
